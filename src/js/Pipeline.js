...
5. corrections_needed에는 블로그에서 수정해야 할 문장의 원본과 수정문을 제시.`,
	...
	// Vision 검증 — 이미지가 비유 매핑/문화 통념과 일관되는지 확인
	...
	async _verifyImageWithVision(imageDataUrl, label) {
		try {
			...
			const result = await ApiClient.callAgent(
				`당신은 비유 블로그 이미지 품질 검증 전문가입니다. 한국 독자 시각에서 이미지가 비유 의미를 명확히 전달하는지 판정합니다.

🚨 FAIL 기준 (하나라도 해당하면 ❌):
1. **비유 의미 깨짐**: 비유의 핵심 개념(예: Mutex='한 명만 사용')이 이미지에서 시각적으로 모순.
2. **한국 사회 통념 충돌**: 한국 문화에서 어색한 조합 (예: 공용 화장실에 남녀 혼합, 학교에 술병 등).
3. **인물 일관성 깨짐**: 본문이 특정 인물('철수와 영희' 등)을 명시했는데 외모/인종/성별이 다름.
4. **본문 비유와 시각 불일치**: 본문 비유 매핑과 이미지가 다른 사물/장소.
5. **명백한 시각 오류**: 사물이 비합리적 배치, 신체 비례 깨짐, 글자 깨짐 (단순 텍스트 박힘은 PASS).

✅ PASS: 비유 의미 명확 + 한국 통념 OK + 본문 비유와 일관.

판정 시 본문 비유 매핑과 이미지를 비교하라. reason은 한 문장으로 구체적 설명.`,
				[[
					{ type: "text", text: `[비유 컨텍스트]
주제: ${ctx.topic || "?"}
비유: ${design.confirmed_analogy || "?"}
세계관: ${(design.worldview || "").substring(0, 200)}
주요 매핑: ${mappingText}
이미지 라벨: ${label}` },
			...
			this._track(result.usage || {});
			return result.data || { pass: true, reason: "" };
		} catch (e) {
			console.warn(`[Vision 검증 실패 ${label}] ${e.message} — PASS로 통과`);
