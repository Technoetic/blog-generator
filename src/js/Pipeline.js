...
출력: 0~4 사이 인덱스 정수 (선택한 후보의 위치) + 선택 사유.`,
	...
	static async _detectAmbiguousTopic(topic, searchData) {
		// fast path 1: 한글+영문 혼합 = 도메인 명시로 간주
		if (topic.split(/\s+/).length >= 2 && /[가-힣]/.test(topic) && /[a-zA-Z]/.test(topic)) {
			return null;
		}
		const hits = searchData?.webResults || [];
		// fast path 2: 검색 결과의 canonical_name이 입력 토픽과 동일하고 결과 5건+이면 모호 아님
		//   — LLM 호출 전 결정론적 컷. 검색이 단일 의미로 수렴했다는 강력한 신호.
		const canonicalName = searchData?.canonicalName;
		if (canonicalName && hits.length >= 5) {
			console.log(`[모호 감지 skip] canonical=${canonicalName}, hits=${hits.length} → 단일 도메인 수렴 (LLM 호출 생략)`);
			return null;
		}
		const webContext = hits.length > 0
			? hits.slice(0, 8).map((r, i) => `[${i + 1}] ${r.title}\n${r.snippet}`).join("\n\n")
			: "검색 결과 없음 (LLM 사전 지식으로 판정)";
		const canonicalHint = canonicalName ? `\n\n[정식 명칭] ${canonicalName}` : "";
		try {
			const result = await ApiClient.callAgent(
...
context_packet의 structure_mapping이 이미 채워져 있어도 더 풍부하게 확장해서 5개 이상으로 출력하라. counterexample_tests는 새로 작성. 하나라도 누락 시 시스템이 FAIL 처리해 재시도가 발생한다.`,
	...
	async _runVerifyA() {
		// 14회차(2026-05-01): A1~A5는 결정론적 코드로 직접 검증.
		//   이전: LLM(Gemini Lite)이 검증 → 멀쩡한 rationale을 보고 "누락" 환각 판정해
		//     Phase 2a가 시도 1회차에서 매번 FAIL → 재시도 빈번.
		//   변경: 필드 존재/배열 길이/문자열 길이는 코드로 검사. A6(본질 매핑)는
		//     Adequacy ⑦이 이미 같은 4차원 평가를 하므로 별도 LLM 호출 생략.
		const d = this.results.design || {};
		const items = [];
		const failReasons = [];

		// A1: fitness_score 정수 ≥ 7
		const fs = d.fitness_score;
		const a1 = Number.isInteger(fs) && fs >= 7;
		items.push({ id: "A1", name: "fitness_score ≥ 7", result: a1 ? "PASS" : "FAIL", reason: a1 ? `${fs}` : `fitness_score=${fs} (정수 7 이상 필요)` });
		if (!a1) failReasons.push(`A1: fitness_score=${fs} (7 이상 필요)`);

		// A2: structure_mapping 배열 길이 ≥ 5
		const sm = Array.isArray(d.structure_mapping) ? d.structure_mapping : [];
		const a2 = sm.length >= 5;
		items.push({ id: "A2", name: "structure_mapping ≥ 5", result: a2 ? "PASS" : "FAIL", reason: `length=${sm.length}` });
		if (!a2) failReasons.push(`A2: structure_mapping 길이 ${sm.length} (5 이상 필요)`);

		// A3: counterexample_tests 배열 길이 ≥ 3
		const ct = Array.isArray(d.counterexample_tests) ? d.counterexample_tests : [];
		const a3 = ct.length >= 3;
		items.push({ id: "A3", name: "counterexample_tests ≥ 3", result: a3 ? "PASS" : "FAIL", reason: `length=${ct.length}` });
		if (!a3) failReasons.push(`A3: counterexample_tests 길이 ${ct.length} (3 이상 필요)`);

		// A4: worldview 1~300자
		const wv = (d.worldview || "").trim();
		const a4 = wv.length >= 1 && wv.length <= 300;
		items.push({ id: "A4", name: "worldview 1~300자", result: a4 ? "PASS" : "FAIL", reason: `length=${wv.length}` });
		if (!a4) failReasons.push(`A4: worldview 길이 ${wv.length} (1~300 필요)`);

		// A5: structure_mapping 각 항목 rationale 1자 이상
		const a5MissingIdx = sm.map((it, i) => (it && typeof it.rationale === "string" && it.rationale.trim().length >= 1 ? -1 : i)).filter((i) => i >= 0);
		const a5 = a5MissingIdx.length === 0 && sm.length > 0;
		items.push({ id: "A5", name: "structure_mapping 모든 rationale 채워짐", result: a5 ? "PASS" : "FAIL", reason: a5 ? "all filled" : `누락 인덱스 ${a5MissingIdx.join(",")}` });
		if (!a5) failReasons.push(`A5: structure_mapping[${a5MissingIdx.join(",")}] rationale 누락`);

		// A6는 Adequacy ⑦이 이미 4차원 본질 매핑을 평가했으므로 그 결과를 재사용.
		const adq = this.results.adequacy;
		const a6 = !adq || adq.adequacy_score === undefined || adq.adequacy_score < 0 || adq.adequacy_score >= 7;
		items.push({ id: "A6", name: "본질 매핑 (Adequacy ⑦ 재사용)", result: a6 ? "PASS" : "FAIL", reason: adq ? `score=${adq.adequacy_score}` : "Adequacy 미실행/스킵" });
		if (!a6) failReasons.push(`A6: 본질 매핑 부족 (Adequacy=${adq.adequacy_score}/10)`);

		const verdict = failReasons.length === 0 ? "PASS" : "FAIL";
		this.results.verifyA = { phase: "2a", verdict, items, fail_summary: failReasons };
		console.log(`[Verify A] 결정론적 검증: ${verdict} (${items.filter((i) => i.result === "PASS").length}/6 PASS)`);
		return { pass: verdict === "PASS", failSummary: failReasons.join(", ") };
	}
...
## 절대 규칙 (렌더링 깨짐 방지)
- **🚨 자기검토/메타 코멘트 본문 출력 절대 금지** — "Mermaid diagram check: 4 nodes, 3 levels, All good", "Node labels: A[기술용어 — 비유대상] format, max 16 chars, All good", "(self-check: ...)" 같은 자기검토 텍스트를 본문 어디에도 박지 마라. <em>/<i>/<small>/quote 태그 안이든 평문이든 모두 금지. 발행되는 글에 노출되면 안 되는 메타데이터다.
...
톤: ${tone} | 비율: ${ratio}`,
	...
	async _validateAndRetryWriter(tone) {
		for (let attempt = 0; attempt < 2; attempt++) {
			...
			// 14회차 2회차(2026-05-01): Writer 출력에서 LLM 자기검토 메타 코멘트 제거.
			//   "Mermaid diagram check: 4 nodes... All good", "Node labels: A[기술용어 — 비유대상] format..."
			//   같은 자기검토 텍스트가 <em>/<i> 태그로 본문에 박혀 발행되던 결함.
			const rawBody = this.results.blog.body || "";
			const cleanedBody = BlogAssembler._stripMetaComments(rawBody);
			if (cleanedBody !== rawBody) {
				console.log(`[Writer] 메타 코멘트 sanitize: ${rawBody.length}자 → ${cleanedBody.length}자`);
				this.results.blog.body = cleanedBody;
			}
			...
			// 14회차(2026-05-01): 잘림 감지를 finish_reason 단독 신호로 단순화.
			//   이전: finish=length OR 마지막 문자가 [.。!?…」』)]가 아니면 incomplete=truncated.
			//     → 본문이 "찾아올게요! 👋" 처럼 이모지/한국어 종결어미로 끝나면 환각 FAIL.
			//   변경: finish_reason="length"만 진짜 잘림. "stop"이면 LLM이 자연스럽게
			//     끝낸 것이므로 신뢰 (이모지/감탄사/한국어 어미는 정상 종결).
			//     백업으로 *명백한 비완결 패턴*만 검사: 마지막이 쉼표/세미콜론/대시로 끝나거나
			//     마지막 토큰이 미완성 마크다운(```, ##, **)일 때만 잘림.
			const finishLength = this.results.blogFinishReason === "length";
			const trimmedTail = body.replace(/\s+$/, "");
			// 명백한 비완결: 쉼표/콜론/대시로 끝남 OR 미완성 마크다운 토큰 (``` 단독 시작, ## 헤더만, ** 미닫힘)
			const danglingPunct = /[,:;\-—]$/.test(trimmedTail) && !/\.\.\.$|…$/.test(trimmedTail);
			const danglingMd = /(```[a-zA-Z]*$)|(^|\n)#{1,6}\s*$/.test(trimmedTail);
			const incomplete = trimmedTail.length > 100 && (danglingPunct || danglingMd);
...
6. (4회차 신설) 본문에 \`\`\`mermaid 다이어그램이 있으면 다이어그램 라벨의 "기술용어 — 비유대상" 매핑이 design.structure_mapping과 일치하는지 확인. 다이어그램 라벨에 등장하는 기술 용어가 design.structure_mapping에 없거나, 본문 설명과 다르게 매핑되어 있으면 mapping_validity에 valid:false로 기록하고 corrections_needed에 수정문 제시.`,
	...
	// 14회차(2026-05-01): 7회→4회 단축, 첫 시도는 Vision skip(첫 이미지 신뢰), 백오프 단축.
	//   이전: 7회 시도 + 119초 대기 + 매회 Vision 검증 → 4분+ 소요(평균 274초).
	//     실측 데이터(Nano Banana 585콜 / 198 클러스터): 평균 2.95콜/편 → 7회는 과잉.
	//   변경:
	//     1) 시도 횟수 7→4 (첫+재시도 3회). 백오프 [0, 1500, 4000, 10000] (총 15.5초).
	//     2) 첫 시도(i=0)는 Vision skip하고 결과를 fallback으로 보존. 2회차부터 검증.
	//        → 정상 케이스: 1회 생성 + 검증 1회 = ~30초.
	//        → Vision 모호 판정 케이스: 첫 결과 즉시 사용(엄격 검증 통과 못해도 좋은 그림 많음).
	//     3) Vision FAIL 누적 시도 시 명시적 "회피" 누적 → 명료한 prompt 추가.
	async _generateImageWithRetry(prompt, ratio, label) {
		this._imgState = this._imgState || {};
		const delays = [0, 1500, 4000, 10000]; // 총 4회 시도, 대기 합계 15.5초
		let lastErr = null;
		const visionFeedbacks = [];
		...
		for (let i = 0; i < delays.length; i++) {
			...
			try {
				...
				if (res.url) {
					// 14회차: 첫 시도는 Vision 검증 skip (Nano Banana 첫 결과 신뢰). 2회차부터만 검증.
					if (i === 0) {
						console.log(`[이미지 ${label}] 1차 시도 — Vision 검증 skip (첫 결과 신뢰)`);
					...
					if (verdict.pass) {
						console.log(`이미지 ${i + 1}차 시도 성공 (${label})`);
						this._imgState[label] = `${label} ✓`;
						this._updateImgSubStatus();
						return res;
					}
		...
		if (bestFallbackRes) {
			console.warn(`[이미지 ${label}] Vision ${delays.length - 1}회 실패, fallback 이미지 사용: ${bestFallbackReason}`);
			this._imgState[label] = `${label} ⚠ (Vision 일치도 낮음, fallback)`;
			this._updateImgSubStatus();
			return bestFallbackRes;
		}
		throw new Error(`이미지 생성 ${delays.length}회 재시도 모두 실패 (${label}): ${lastErr}`);
	}
	...
	async _verifyImageWithVision(imageDataUrl, label) {
		...
		for (let attempt = 0; attempt < MAX_API_RETRIES; attempt++) {
			try {
				const result = await ApiClient.callAgent(
					`당신은 비유 블로그 이미지 품질 검증 전문가입니다. 비유 일러스트는 사실주의 사진이 아니라 *상징적 그림*임을 인지하고 *결정적 위반만* 판정합니다.

🚨 FAIL 기준 (다음 중 하나가 *명백히* 위반될 때만 ❌. 애매하면 PASS):

**1. 비유 의미 정면 모순 (단 하나의 명백한 사례만)**
   비유의 핵심 개념을 이미지가 *반대로* 보여주는 경우만.
   예: Mutex 비유('한 번에 한 명만')인데 *여러 명이 동시에 같은 자원을 사용*하는 모습.
   ⚠ 단순히 인원이 많거나 배경이 다른 것은 PASS. 의미가 *반대*일 때만 FAIL.

**2. 본문 핵심 사물/장소 완전 부재**
   본문 매핑(예: '화장실↔공유자원')의 *주요 사물*이 이미지에 *전혀 등장 안 함*.
   ⚠ 색상/배경/세부 묘사 차이는 PASS. 핵심 매핑 사물이 *없을 때만* FAIL.

**3. 명백한 신체 비례 깨짐 / 사물 비합리적 배치**
   사람 손가락 6개, 머리 2개 같은 명백한 그림 오류만.
   ⚠ 약간의 어색함, 양식화는 PASS.

✅ PASS 가이드: 비유 의미가 *대체로* 전달되고 본문 핵심 사물이 *있으면* 통과.
   - 영문 텍스트 박힘(VACANT/OCCUPIED 등) → PASS
   - 인물 외모/인종 차이 → PASS (비유 일러스트라 일관성보다 메시지 우선)
   - 공용 시설 성별 혼재 → PASS (현실 사진이 아님)
   - 색상/스타일/배경 자유도 → PASS

판정 절차:
1) 비유의 *핵심 의미* 한 문장 파악
2) 이미지가 그 의미를 *반대로 보여주는지* 확인 (1번)
3) 핵심 사물이 *전혀 없는지* 확인 (2번)
4) 명백한 그림 오류만 확인 (3번)
5) 셋 다 OK면 PASS

reason은 한 문장. 모호하면 PASS 쪽으로 기울여라.`,
					[[
