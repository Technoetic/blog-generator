...
This project is an **end-to-end AI blog generation platform** that converts a single technical keyword into a fully illustrated Korean blog post published to Blogger.

A **6-agent orchestration pipeline** — Analogy Designer / Blog Writer / Image Prompter / Verifier / Evaluator / Fact-Checker — runs sequentially with parallel sub-phases, and each agent has its own thinking budget and temperature configuration optimized on Gemini 2.5 Flash (Lite). Every `mermaid` code block generated by the writer is rasterized client-side via **Excalidraw's hand-drawn renderer** with a Korean handwritten font (Gaegu) and uploaded to Imgur for embedding.

The verifier agent enforces a 4-stage quality gate (inspect → measure → evidence → verdict), the fact-checker calls Google Search via function-calling to verify every technical claim, and the evaluator scores the final blog on 7 rubrics. Failed agents are retried individually up to 2–3 times with failure feedback. The final post is auto-published via **Blogger API v3** using a long-lived refresh token flow.
