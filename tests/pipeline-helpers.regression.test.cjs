...
describe("14회차 2회차: Vision 7회 실패 시 bestFallbackRes 사용 (발행 진행 보장)", () => {
	...
	assert(/이미지 생성 \$\{delays\.length\}회 재시도 모두 실패/.test(slice),
		"이미지 자체 없는 경우만 throw 메시지 (회수 동적)");
	// fallback 사용 시 일치도 낮음 표시
	assert(/Vision 일치도 낮음|Vision \$\{delays\.length - 1\}회 실패, fallback 이미지 사용/.test(slice),
		"fallback 사용 시 사용자/로그 표시 (회수 동적)");
});
